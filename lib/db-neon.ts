...
export async function addMember(member: Omit<Member, 'id'>): Promise<Member> {
  ...
  const rows = await (sql as any)`
    INSERT INTO family_members (name, gender, birth, birth_place, dead, death_info, note, parents, children, spouses)
    VALUES (
...
export async function updateMember(id: number, updates: Partial<Member>): Promise<Member | null> {
  ...
  const rows = await (sql as any)`
    UPDATE family_members SET
      ...
      parents = ${m.parents || []}, children = ${m.children || []},
      spouses = ${m.spouses || []}, updated_at = NOW()
    WHERE id = ${id}
    RETURNING *
  ` as any[];
...
async function seedInitialData() {
  ...
  for (const m of initialMembers) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    await (sql as any)`
      INSERT INTO family_members (id, name, gender, birth, birth_place, dead, death_info, note, parents, children, spouses)
      VALUES (
        ${m.id}, ${m.name}, ${m.gender || ''}, ${m.birth || ''}, ${m.birthPlace || ''},
        ${m.dead || false}, ${m.deathInfo || ''}, ${m.note || ''},
        ${m.parents}, ${m.children}, ${m.spouses}
      )
